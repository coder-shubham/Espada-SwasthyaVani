### we can run this directly from inside of docker container
...
vector_db_client = _get_vector_db_client()
...
coll = WeaviateCollectionClient(db_client=vector_db_client, name=GOV_SCHEMES_COLL_NAME, embeddings=embeddings)
...
for scheme_name, url in GOV_SCHEMES.items():
    ...
    for doc in docs:
        ...
        vector = embeddings.encode(properties['text']).tolist()
